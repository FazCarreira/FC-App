...
import { empresaPerfilIn } from '../../constants';
...
const EmpresaProfile = () => {
    ...
    const handleNext = () => {
        if (!empresaPerfilIn[activeStep].some((v) => formData?.[v.name] === '')) {
    ...
    if (profile) return <Navigate to="/parceiro/inicio" />
    return (<>
        <ModalForm
            title='Termos de uso'
        ...
        <Box sx={{ my: 4 }}>
            ...
            <Fragment>
                <Grid container spacing={3}>
                    {empresaPerfilIn[activeStep].map(q => <Form key={q.name} question={q} value={formData[q.name]} onChange={activeStep === 0 ? onChange : onChangeBank} />)}
                </Grid>
